...
{var filterDef={"columns":[],"bootstrap":!1};if(("bootstrap" in initArray)&&(typeof initArray.bootstrap==='boolean'))
...
{var idx=initColumn.idx;filterDef.columns[idx]={"cssStyle":"","cssClass":(filterDef.bootstrap)?"form-control ":"","titleOverride":null};if(('cssStyle' in initColumn)&&(typeof initColumn.cssStyle==='string'))
...
{if(e.namespace!=='dt'){return}
var api=new $.fn.dataTable.Api(settings);var id=api.table().node().id;var initObj=api.init();if(!("filterDropDown" in initObj))return;var filterDef=parseInitArray(initObj.filterDropDown);var container=api.table().container();api.columns(Object.keys(filterDef.columns)).every(function()
...
{select.append('<option value="'+d+'">'+d+'</option>')})})})}(jQuery))